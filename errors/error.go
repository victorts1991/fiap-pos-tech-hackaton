...
func getHttpCode(err *errorx.Error) int {
	switch {
	case err.IsOfType(errorx.IllegalFormat) || err.IsOfType(errorx.IllegalArgument) || err.IsOfType(BadRequest):
		return 400
	case err.IsOfType(NotFound):
		return 404
	case err.IsOfType(Unauthorized):
		return 401
	default:
		return 500
	}
